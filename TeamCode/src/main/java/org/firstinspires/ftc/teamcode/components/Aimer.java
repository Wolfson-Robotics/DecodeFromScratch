...
public class Aimer extends RollerEx {
    ...
    public double GEAR_RATIO = 92.0 / 200.0;
    ...
    public void aim(double yaw) {
        //if (imu == null || controller == null) { return; }

        encoderPos = motor.getCurrentPosition();
        robotHeading = imu.getRobotYawPitchRollAngles().getYaw(AngleUnit.DEGREES);
        localAngle = (encoderPos / TICKS_PER_REV) * 180 * GEAR_RATIO;
        globalAngle = localAngle + robotHeading / 2;

        targetAngle = yaw;

        targetPos = (int)(TICKS_PER_REV * (targetAngle + (int)robotHeading/2.0) / 180 * GEAR_RATIO);
        power = controller.calculate(encoderPos, targetPos);
